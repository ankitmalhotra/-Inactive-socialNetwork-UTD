...
@end
@implementation ax23_SQLException
- (id)init
{
	if((self = [super init])) {
		SQLState = 0;
...
- (xmlNodePtr)xmlNodeForDoc:(xmlDocPtr)doc elementName:(NSString *)elName elementNSPrefix:(NSString *)elNSPrefix
{
	...
	{
		nodeName = [NSString stringWithFormat:@"%@:%@", @"ax23", elName];
	}
	...
	xmlSetNsProp(node, xsi, (const xmlChar*)"type", (const xmlChar*)"ax23:SQLException");
...
- (void)addElementsToNode:(xmlNodePtr)node
{
	...
	if(self.SQLState != 0) {
		xmlAddChild(node, [self.SQLState xmlNodeForDoc:node->doc elementName:@"SQLState" elementNSPrefix:@"ax23"]);
	}
	if(self.errorCode != 0) {
		xmlAddChild(node, [self.errorCode xmlNodeForDoc:node->doc elementName:@"errorCode" elementNSPrefix:@"ax23"]);
	}
	if(self.nextException != 0) {
		xmlAddChild(node, [self.nextException xmlNodeForDoc:node->doc elementName:@"nextException" elementNSPrefix:@"ax23"]);
	}
}
/* elements */
@synthesize SQLState;
...
{
	ax23_SQLException *newObject = [[ax23_SQLException new] autorelease];
