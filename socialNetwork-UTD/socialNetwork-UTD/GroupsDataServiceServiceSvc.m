...
#endif
@implementation GroupsDataServiceServiceSvc_SQLException
- (id)init
{
	if((self = [super init])) {
		SQLException = 0;
...
- (void)dealloc
{
	if(SQLException != nil) [SQLException release];
...
- (void)addElementsToNode:(xmlNodePtr)node
{
	...
	if(self.SQLException != 0) {
		xmlAddChild(node, [self.SQLException xmlNodeForDoc:node->doc elementName:@"SQLException" elementNSPrefix:@"GroupsDataServiceServiceSvc"]);
	}
}
/* elements */
@synthesize SQLException;
/* attributes */
...
{
	GroupsDataServiceServiceSvc_SQLException *newObject = [[GroupsDataServiceServiceSvc_SQLException new] autorelease];
...
- (void)deserializeElementsFromNode:(xmlNodePtr)cur
{
	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(xmlStrEqual(cur->name, (const xmlChar *) "SQLException")) {
				
				Class elementClass = nil;
				xmlChar *instanceType = xmlGetNsProp(cur, (const xmlChar *) "type", (const xmlChar *) "http://www.w3.org/2001/XMLSchema-instance");
				if(instanceType == NULL) {
					elementClass = [ax23_SQLException class];
				} else {
	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(xmlStrEqual(cur->name, (const xmlChar *) "userDetailsobject")) {
				...
				xmlChar *instanceType = xmlGetNsProp(cur, (const xmlChar *) "type", (const xmlChar *) "http://www.w3.org/2001/XMLSchema-instance");
				if(instanceType == NULL) {
					elementClass = [ax25_UserDetails class];
				} else {
					NSString *elementTypeString = [NSString stringWithCString:(char*)instanceType encoding:NSUTF8StringEncoding];
	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(xmlStrEqual(cur->name, (const xmlChar *) "return")) {
				...
				xmlChar *instanceType = xmlGetNsProp(cur, (const xmlChar *) "type", (const xmlChar *) "http://www.w3.org/2001/XMLSchema-instance");
				if(instanceType == NULL) {
					elementClass = [ax25_GroupDetails class];
				} else {
					NSString *elementTypeString = [NSString stringWithCString:(char*)instanceType encoding:NSUTF8StringEncoding];
...
+ (void)initialize
{
	...
	[[USGlobals sharedInstance].wsdlStandardNamespaces setObject:@"ax25" forKey:@"http://GetGroupsInformation/xsd"];
	[[USGlobals sharedInstance].wsdlStandardNamespaces setObject:@"ax23" forKey:@"http://sql.java/xsd"];
}
...
- (NSString *)serializedFormUsingHeaderElements:(NSDictionary *)headerElements bodyElements:(NSDictionary *)bodyElements
{
	...
	xmlNewNs(root, (const xmlChar*)"http://GetGroupsInformation/xsd", (const xmlChar*)"ax25");
	xmlNewNs(root, (const xmlChar*)"http://sql.java/xsd", (const xmlChar*)"ax23");
